...
import { Animated, View, Dimensions, PanResponder, Text } from "react-native";
...
export default React.memo(function BottomSheet(props: BottomSheetProps) {
  ...
  const responder = React.useRef(
    PanResponder.create({
      ...
      onPanResponderRelease: (_, gesture) => {
        ...
        if (lastGestureDy.current < maxUpwardTranslateY) lastGestureDy.current = maxUpwardTranslateY;
        else if (lastGestureDy.current > maxDownwardTranslateY) lastGestureDy.current = maxDownwardTranslateY;
  ...
  return (
    <Animated.View style={[
      styles.container,
      animation, { height: maxHeight, bottom: minHeight - maxHeight },
      type === "1" && styles.type1,
      type === "2" && styles.type2
    ]}>
